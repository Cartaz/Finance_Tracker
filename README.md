...
Milestones M0 through M6 are implemented on the current M6 branch and are under final validation before merge to `main`.
...
- usable Dashboard, Transactions, Accounts and Reconciliation views;
...
- SQLite with verified foreign-key enforcement, WAL mode and migrations through schema v5;
- currencies, users/books, accounts, transactions, entries, payees, aliases, historical book-scoped FX rates, CSV import staging rows and reconciliation identities;
- exact money parsing using integer minor units and `Decimal`; financial `float` values are rejected;
- `AccountService`, `LedgerService`, `BookService`, `PayeeService`, `CategoryService`, `FxService`, read-only `ReportingService` and zero-trust `ReconciliationService`;
...
- permanent deterministic stress suites across M0-M6, including malformed input, cross-book references, rollback, integrity/foreign-key checks, missing FX, cross-currency operations, reconciliation duplicates/ambiguity and reporting read-only invariants.

Later V1 milestones cover scheduled transactions, budgets, forecasting, loans/financing and complete backup/restore UX.
