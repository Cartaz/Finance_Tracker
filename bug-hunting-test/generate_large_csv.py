...
def row_for(index: int) -> list[str]:
    ...
    return [
        ...
        f"BUG-HUNT-{index:05d}",
    ]


def write_fixture(path: Path, row_count: int) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(HEADERS)
        for index in range(1, row_count + 1):
            writer.writerow(row_for(index))
...
def main() -> None:
    ...
    write_fixture(
        destination / "bug-hunting-test-reconciliation-10000-valid.csv",
        10_000,
    )
    write_fixture(
        destination / "bug-hunting-test-reconciliation-10001-too-many.csv",
        10_001,
    )
