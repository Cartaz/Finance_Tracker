...
def test_category_tree_and_paths(ledger_env) -> None:
    ...
    assert service.category_path(ledger_env.book_id, groceries.id) == "Alimentazione › Supermercato"
...
def test_category_autocomplete_prefers_payee_history(ledger_env) -> None:
    ...
    supermarket = categories.create_category(book_id=book, category_type="EXPENSE", name="Supermercato")
    subscriptions = categories.create_category(book_id=book, category_type="EXPENSE", name="Subscriptions")
    ...
    for index in range(4):
        ...
        payees.assign_transaction(book_id=book, transaction_id=tx.id, payee_id=amazon.id)
