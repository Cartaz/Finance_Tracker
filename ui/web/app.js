(() => {
  ...
  function renderBudgets(report) {
    ...
    $("budget-list").innerHTML = report.budgets.map((item) => `<div class="card"><div class="report-row"><b>${escapeHtml(item.categoryPath)}</b><span>${item.transactionCount} movimenti · ${percentBps(item.usageBps)}</span><span>${money(item.spentMinor, currency)} / ${money(item.amountMinor, currency)}</span><strong>${item.remainingMinor == null ? "FX mancante" : `${item.overBudget ? "Oltre di " : "Residuo "}${money(item.overBudget ? -BigInt(String(item.remainingMinor)) : item.remainingMinor, currency)}`}</strong></div><div class="history-controls"><button type="button" data-budget-delete="${item.id}">Elimina</button></div></div>`).join("") || `<p class="empty">Nessun budget per questo mese.</p>`;
  }
