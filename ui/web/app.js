(() => {
  ...
  const money = (minor, currency = "EUR") => { const formatter = new Intl.NumberFormat("it-IT", { style: "currency", currency }); const digits = formatter.resolvedOptions().maximumFractionDigits; return formatter.format((minor || 0) / (10 ** digits)); };
  ...
  function render(snapshot) {
    ...
    const sameCurrency = balanceAccounts.filter((a) => a.currency === snapshot.book.currency);
    const net = sameCurrency.reduce((sum, a) => sum + (a.balanceMinor || 0), 0);
    $("metrics").innerHTML = `<article class="card metric"><span>Saldo conti in ${snapshot.book.currency}</span><strong>${money(net, snapshot.book.currency)}</strong></article><article class="card metric"><span>Conti</span><strong>${balanceAccounts.length}</strong></article><article class="card metric"><span>Transazioni recenti</span><strong>${snapshot.transactions.length}</strong></article>`;
